...
import { resolveWinuxshPath } from './packages/shell/winuxsh-local/src/resolve.ts'
...
// winuxsh-local's run/start/lifecycle suites self-skip without a real winuxsh
// binary (executor.spec.ts hasWinuxsh), leaving those files far below per-file
// 100% on winuxsh-less hosts; the exemption mirrors the pwsh pattern — the
// probe runs the suites' own resolution, so the exemption is active exactly
// when the suites skip.
const winuxshCoverageExclusions = spawnSync(resolveWinuxshPath(), ['-c', 'exit 0'], { encoding: 'utf8' }).status === 0
  ? []
  : [
      'packages/shell/winuxsh-local/src/index.ts',
      'packages/shell/winuxsh-sandbox/src/**/*.ts',
    ]
