 ...
 * Exported so the winuxsh twin can type its overrides against the same
 * resolved-section shape.
 */
export type ResolvedConfig = Required<Omit<Config, 'cwd' | 'pwshPath'>> & Pick<Config, 'cwd' | 'pwshPath'>
 ...
 * the next command. The message prefix is parameterized so the winuxsh twin
 * reuses the identical checks under its own diagnostics.
 * @param config - the resolved section, schema-valid by construction.
 * @param prefix - diagnostic prefix for the thrown messages (default `pwsh-local`).
 * @throws Error naming the field that cannot be used.
 */
export function assertServiceablePwshConfig(config: Config, prefix = 'pwsh-local'): void {
  const resolved = config as ResolvedConfig
  assertPositiveFinite(`${prefix}: timeoutMs`, resolved.timeoutMs)
  assertPositiveFinite(`${prefix}: maxTimeoutMs`, resolved.maxTimeoutMs)
  assertPositiveFinite(`${prefix}: maxOutputBytes`, resolved.maxOutputBytes)
  assertPositiveFinite(`${prefix}: maxSpillBytes`, resolved.maxSpillBytes)
  assertPositiveFinite(`${prefix}: graceMs`, resolved.graceMs)
  if (resolved.graceMs > MAX_TIMER_DELAY_MS) {
    throw new Error(`${prefix}: graceMs must be no greater than ${MAX_TIMER_DELAY_MS}`)
  }
}
...
export class PwshLocalExecutor extends ShellExecutor {
  ...
  /** Diagnostic prefix used in executor-thrown messages. */
  protected readonly diagnosticPrefix: string = 'pwsh-local'
  ...
  /**
   * The declared executable from a resolved section — the value a subclass's
   * config names in place of `pwshPath` (see `@deepseek-ai/dsh-winuxsh-local`).
   * @param config - the resolved section.
   * @returns the declared executable, or undefined for discovery.
   */
  protected declaredExecutable(config: ResolvedConfig): string | undefined {
    return config.pwshPath
  }

  /**
   * Resolve the declared executable, or discover the platform default.
   * @param declared - the declared executable from {@link declaredExecutable}.
   * @returns the exact executable every command runs through.
   */
  protected resolveExecutable(declared: string | undefined): string {
    return resolvePwshPath(declared)
  }

  /**
   * Reject a resolved section this executor could not run with. Subclasses
   * override for their own diagnostic prefix.
   * @param config - the resolved section, schema-valid by construction.
   */
  protected assertServiceable(config: Config): void {
    assertServiceablePwshConfig(config)
  }

  /**
   * Model-friendly environment overrides merged under the caller's own entries
   * on every spawn. Subclasses extend for their shell dialect.
   */
  ...
  constructor(ctx: Context, config: Config) {
    ...
    this.assertServiceable(entry)
    this.source = () => entry
    const declared = this.declaredExecutable(entry)
    this.declaredPwshPath = declared
    this.resolvedPwshPath = this.resolveExecutable(declared)
    // The subclass's own static Config (through `this.constructor`) is the
    // section schema: the winuxsh twin registers its `winuxshPath` keys here.
    installSettingsSection(ctx, SHELL_SETTINGS_NAMESPACE, (this.constructor as typeof PwshLocalExecutor).Config, entry, {
      validate: (value) => this.assertServiceable(value),
      setSource: (current) => {
      ...
      onChange: () => {
        const declared = this.declaredExecutable(this.source())
  ...
  resolve(request: ShellExecRequest): ShellExecSpec {
    const timeoutMs = clampTimeout(
      ...
      `${this.diagnosticPrefix}: request.timeoutMs`,
    )
    const stdoutMaxBytes = request.stdoutMaxBytes ?? this.config.maxOutputBytes
    assertPositiveFinite(`${this.diagnosticPrefix}: request.stdoutMaxBytes`, stdoutMaxBytes)
    return {
  ...
  private collected(handle: SubprocessHandle): { stdout: SubprocessOutputReader; stderr: SubprocessOutputReader } {
    ...
    if (stdout === undefined || stderr === undefined) {
      throw new Error(`${this.diagnosticPrefix}: subprocess implementation dropped a requested collect stream`)
    }
    /* v8 ignore stop */
