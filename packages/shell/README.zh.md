...
| [`pwsh-sandbox/`](pwsh-sandbox/README.md) | 对 PowerShell 命令应用已配置的 [`sandbox`](../sandbox/README.md) 后端。 | （注册 `ctx.shell`） |
| [`winuxsh-local/`](winuxsh-local/README.md) | 通过 Windows 原生的 bash 兼容 `winuxsh -c` shell 执行命令，继承 pwsh 执行器机制。 | （注册 `ctx.shell`） |
| [`winuxsh-sandbox/`](winuxsh-sandbox/README.md) | 对 winuxsh 命令应用已配置的 [`sandbox`](../sandbox/README.md) 后端（`bash-sandbox` 的 Windows 孪生）。 | （注册 `ctx.shell`） |
