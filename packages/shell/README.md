...
| [`pwsh-sandbox/`](pwsh-sandbox/README.md) | Applies the configured [`sandbox`](../sandbox/README.md) backend to PowerShell commands. | (registers `ctx.shell`) |
| [`winuxsh-local/`](winuxsh-local/README.md) | Executes commands through the Windows-native bash-compatible `winuxsh -c` shell, inheriting the pwsh executor mechanics. | (registers `ctx.shell`) |
| [`winuxsh-sandbox/`](winuxsh-sandbox/README.md) | Applies the configured [`sandbox`](../sandbox/README.md) backend to winuxsh commands (the Windows twin of `bash-sandbox`). | (registers `ctx.shell`) |
