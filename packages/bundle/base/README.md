...
The patch gates the shell executor by platform on its own rows: `bash-sandbox` mounts on POSIX only (`disabled: !!js process.platform === 'win32'` — bash has no Windows runner), and its Windows twin `winuxsh-sandbox` (a Windows-native bash-compatible shell) mounts on win32 only with the inverted expression; the retired `pwsh-sandbox`/`tool-pwsh` rows are fully disabled, and the model-facing `tool-bash` carries no platform gate so it runs through either executor — one shared patch file, exactly one shell executor per host. The permission surface stays exactly as on POSIX: `sandbox`/`sandbox-policy` enforce the file-effect policy through the Windows ACL restricted-token runner (the win32 chain of `dsh-sandbox-local` → `@deepseek-ai/dsh-sandbox-windows-acl`), the permission switcher and the approval service run unchanged, and `fs-sandbox` keeps fencing `ctx.fs` writes — mounting `dsh-fs-local` alongside it would double-register `ctx.fs` and fail the load. A Windows host that prefers the unconfined local pwsh executor or full access overrides these rows through its profile or home `cordis.patch.yml` (the bash-restore recipe must be complete: disable `winuxsh-sandbox` AND re-enable `bash-sandbox` — both executor families register the same `bash` service, so an incomplete recipe fails loud at load). POSIX hosts see the winuxsh and pwsh executor rows disabled.
