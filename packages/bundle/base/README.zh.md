...
patch 在自身上按平台门控 shell 执行器：`bash-sandbox` 仅在 POSIX 挂载（`disabled: !!js process.platform === 'win32'`——bash 没有 Windows runner），它的 Windows 孪生 `winuxsh-sandbox`（Windows 原生的 bash 兼容 shell）以取反的表达式仅在 win32 挂载；已退役的 `pwsh-sandbox`/`tool-pwsh` 行被完全禁用，而面向模型的 `tool-bash` 不携带平台门控，因此它经任一执行器运行——同一份 patch 文件，每个宿主恰好挂载一个 shell 执行器。权限面与 POSIX 完全一致：`sandbox`/`sandbox-policy` 通过 Windows ACL 受限令牌 runner（`dsh-sandbox-local` 的 win32 链 → `@deepseek-ai/dsh-sandbox-windows-acl`）执行文件效果策略，权限切换器与 approval 服务原样运行，`fs-sandbox` 继续围栏 `ctx.fs` 写入——在其旁再挂载 `dsh-fs-local` 会重复注册 `ctx.fs` 并在加载时失败。偏好不受沙盒约束的本地 pwsh 执行器或完整访问的 Windows 主机通过其 profile 或 home 的 `cordis.patch.yml` 覆盖这些行（bash 恢复配方必须完整：禁用 `winuxsh-sandbox` 并重新启用 `bash-sandbox`——两个执行器家族注册同一个 `bash` 服务，配方不完整会在加载时直接报错）。POSIX 主机看到的是被禁用的 winuxsh 与 pwsh 执行器行。
