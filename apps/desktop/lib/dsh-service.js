...
export function buildDshArgs(entry, { platform = process.platform, windowsPickerPatch = resolveWindowsPickerPatch(), winuxshPatch = resolveWinuxshPatch(), } = {}) {
    return [
        '--expose-internals',
        entry,
        '--profile',
...
export function startDshService({ electronExecutable, entry = resolveDshEntry(), environment = process.env, platform = process.platform, timeoutMs = 60_000, } = {}) {
    ...
    const ready = new Promise((resolve, reject) => {
        ...
        child.once('exit', (code, signal) => finish(reject, new Error(`DeepSeek Harness stopped before it was ready (code ${String(code)}, signal ${String(signal)}).\n${output}`)));
